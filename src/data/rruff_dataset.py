...
class RRUFFOnlineMixingDataset(data.Dataset):
    def __init__(self, rruff_db_path, min_k=2, max_k=4, k_weights=None, min_weight=0.1, weight_distribution="random", target_length=3500, theta_min=10.0, theta_max=80.0, split='all', num_folds=5, fold=0, seed=42):
    ...
    def __len__(self):
        return 1000  # Virtual epoch length since it's online mixing
    ...
    def __getitem__(self, idx):
        ...
        if self.k_weights is None:
            ...
            k = random.choices(range(self.min_k, self.max_k + 1), weights=self.k_weights)[0]
            
        chosen = random.sample(self.phases, k)
...
def create_rruff_dataloader(rruff_db_path, batch_size=32, min_k=2, max_k=4, k_weights=None, min_weight=0.1, weight_distribution="random", target_length=3500, split='all', num_folds=5, fold=0, seed=42, distributed=False, num_workers=4, pin_memory=True):
    dataset = RRUFFOnlineMixingDataset(
