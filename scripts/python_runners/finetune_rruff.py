...
def parse_args():
    ...
    p.add_argument("--fold", type=int, default=0, help="Fold index (0-4) for K-Fold eval.")
    p.add_argument("--num_folds", type=int, default=5, help="Number of total folds.")
...
def main():
    # Ignore PyTorch loader warnings
    import warnings
    ...
    train_loader = create_rruff_dataloader(
        args.singlephase_xrd_db, batch_size=args.batch_size, min_k=2, max_k=args.num_phases,
        split='train', num_folds=args.num_folds, fold=args.fold, seed=args.seed, distributed=True
    )
    val_loader = create_rruff_dataloader(
        args.singlephase_xrd_db, batch_size=args.batch_size, min_k=2, max_k=args.num_phases,
        split='val', num_folds=args.num_folds, fold=args.fold, seed=args.seed, distributed=True
    )
