...
def run_one_epoch(model, dataloader, device, entries_dict, threshold=0.3, save_path='inference_results.json', limit=0):
    ...
    with open(save_path, 'w') as f:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # --- Quant MAE Calculation (Sample-wise) ---
            valid_gt_indices = torch.where(valid_gt_mask)[0]
            if len(row_ind) > 0 and len(col_ind) > 0 and len(row_ind) == len(col_ind):
                # Get matched patterns for the sample
                matched_pred_patterns = pred_xrds[b, row_ind]
                matched_gt_patterns = gt_xrds[b, valid_gt_indices[col_ind]]

                # Use the dedicated function to calculate quant_mae for the matched set
                quant_res = calculate_quantitative_metrics(
                    matched_pred_patterns.unsqueeze(0), 
                    matched_gt_patterns.unsqueeze(0)
                )
                metrics['quant_mae_sum'] += quant_res['quant_mae']
            ...
            for r, c in zip(row_ind, col_ind):
                ...
                # Calculate interference for SIR/SAR (sum of other valid GT patterns)
                interference = gt_xrds[b][valid_gt_mask].sum(dim=0) - gt_xrds[b, gt_idx]
    ...
    if metrics['total_phases'] > 0:
        ...
        print(f"Avg Quant MAE:   {metrics['quant_mae_sum']/metrics['total_samples']:.2f}%")
...
def main():
    ...
    testset = ASEDataset(args.db_path, args.npz_dir, mode='test', 
                         encode_element=args.atom_embed, num_classes=args.num_classes, config=config)
