...
def run_one_epoch(model, dataloader, device, entries_dict):
    ...
    for batch in dataloader:
        ...
        pred_cs = [entries_dict[str(key.item())]['cry_sys'] for key in preds]
        label_cs = [entries_dict[str(key.item())]['cry_sys'] for key in label_cls]

        # Store mapped `sg` values for metric calculations
        all_preds.extend(pred_cs)
        all_labels.extend(label_cs)

        # Count correct predictions
        correct_sg_cnt = sum(p == l for p, l in zip(pred_cs, label_cs))
