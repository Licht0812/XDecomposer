...
class ImpurGen(object):
    ...
    def __init__(self, struc, impur_amt, ref_dir='References', min_angle=10.0, max_angle=80.0):
        ...
        # Optimization: only load if total references are small to avoid O(N^2) complexity
        self.saved_patterns = []
        if os.path.isdir(self.ref_dir):
            if len(os.listdir(self.ref_dir)) < 100: # Threshold for impurity loading
