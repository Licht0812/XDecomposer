...
class SpectrumAnalyzer(object):
    ...
    def get_reduced_pattern(self, predicted_cmpd, orig_y, last_normalization=1.0):
        ...
        # Attempt BGMN-based refinement; fall back to internal simulator on failure
        use_refinement = os.environ.get('AUTOXRD_DISABLE_REFINEMENT', '0') != '1'
        scaled_spectrum, scaling_constant = None, None
        if use_refinement:
            try:
        ...
        # Fallback: simulate phase pattern and scale without BGMN
        if scaled_spectrum is None:
            pred_y = self.generate_pattern(predicted_cmpd)
            scaled_spectrum, scaling_constant = self.scale_spectrum(pred_y, y_obs)
            scaled_spectrum = 1.25*np.array(scaled_spectrum)
