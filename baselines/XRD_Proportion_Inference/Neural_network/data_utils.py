...
def load_npz_pattern(file_path: str, key_priority: List[str] = None) -> Optional[np.ndarray]:
    ...
    try:
        # mmap_mode speeds up random reads
...
class XRDBaseDataset(data.Dataset):
    ...
    def _build_or_load_index(self, use_cache: bool) -> Dict[int, List[str]]:
        ...
        for file_path in all_files:
            ...
            try:
                cid = -1
                # Support multiple crystal ID formats
                if name.startswith("crystal_"):
                    ...
                    # Try plain numeric filenames
                    id_str = name.split('.')[0]
                    if id_str.isdigit(): cid = int(id_str)
                    elif '_' in id_str:
                        # Handle numeric filenames with suffixes
        ...
        # Drop IDs without valid samples
        valid_index = {}
    ...
    def get_crystal_patterns(self, crystal_id: int, max_samples: int = 1) -> List[np.ndarray]:
        ...
        # Retry a few times for valid samples
        random.shuffle(paths)
        patterns = []
        for p in paths:
            arr = load_npz_pattern(p)
            if arr is not None and np.nanmax(arr) > 1e-6: # Require visible signal
                patterns.append(arr)
